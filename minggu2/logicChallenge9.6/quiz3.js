...
function countProfit(shoppers) {
  ...
  for (const pembeli of shoppers) {
    const namaBarang = pembeli.product;
    const jumlahBeli = pembeli.amount;
    const dataBarang = listBarang.find((barang) => barang[0] === namaBarang);

    if (!dataBarang) {
      continue;
    }

    const [nama, harga, stokAwal] = dataBarang;

    if (jumlahBeli > stokAwal) {
      continue;
    }

    const totalProfit = harga * jumlahBeli;
    dataBarang[2] -= jumlahBeli;
    hasil.push({
      product: nama,
      shoppers: [pembeli.name], // Menyimpan nama pembeli
      leftOver: dataBarang[2], // Sisa stok
      totalProfit,
    });
  }
  return hasil;
